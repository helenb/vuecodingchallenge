...
COPY ./vuecodingchallenge/static_src/ ./vuecodingchallenge/static_src/
...
RUN useradd vuecodingchallenge -m && mkdir /app && chown vuecodingchallenge /app
...
ENV PATH=$PATH:${POETRY_HOME}/bin \
    PYTHONUNBUFFERED=1 \
    PYTHONPATH=/app \
    DJANGO_SETTINGS_MODULE=vuecodingchallenge.settings.production \
    PORT=8000 \
    WEB_CONCURRENCY=3 \
...
COPY --chown=vuecodingchallenge pyproject.toml poetry.lock ./
RUN if [ "$BUILD_ENV" = "dev" ]; then poetry install --extras gunicorn; else poetry install --no-dev --extras gunicorn; fi

COPY --chown=vuecodingchallenge --from=frontend ./vuecodingchallenge/static_compiled ./vuecodingchallenge/static_compiled

# Copy application code.
COPY --chown=vuecodingchallenge . .
...
# Load shortcuts
COPY ./docker/bashrc.sh /home/vuecodingchallenge/.bashrc

# Don't use the root user as it's an anti-pattern and Heroku does not run
# containers as root either.
# https://devcenter.heroku.com/articles/container-registry-and-runtime#dockerfile-commands-and-runtime
USER vuecodingchallenge

# Run the WSGI server. It reads GUNICORN_CMD_ARGS, PORT and WEB_CONCURRENCY
# environment variable hence we don't specify a lot options below.
CMD gunicorn vuecodingchallenge.wsgi:application
